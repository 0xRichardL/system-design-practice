...
- Core operation:
  - Notification service receives a notification request from internal services and routes the message to the correct channel & user.
...
- Asynchronous processing, no synchronous API within the system.
...
#### 4.2 Notification delivery duration
...
### 2. Backpressure and overload handling

The system uses queues to absorb short traffic bursts. Backpressure prevents workers and external providers from being overloaded when traffic exceeds processing capacity.

#### Limits

| Type | Push | Email | SMS |
| --- | ---: | ---: | ---: |
| Capacity with headroom | 8,100/s | 2,320/s | 1,160/s |
| Shared provider rate limit | 9,000/s | 2,250/s | 900/s |
| Required CCR with headroom | 810 | 696 | 580 |
| Per-instance concurrency limit | 30 | 30 | 30 |
| Maximum worker instances | 27 | 24 | 20 |
| Actual fleet concurrency limit | 810 | 720 | 600 |
| Broker prefetch per instance | 30 | 30 | 30 |

#### Provider rate limiting

Each channel has shared token bucket stored in Redis. The limiter applies across all worker instances for the same channel.

A worker must obtain 1 token before start the process, if no token is available:

1. Worker does not call the provider.
2. The message remain durable in the Broker.
3. Consumption pauses until capacity becomes available.
4. `RetryCount` is not incremented since this is not delivery failure.

#### Bounded worker concurrency

Each worker instance allows at most 30 concurrent provider requests. Broker prefetch is also limited to 30 messages per instance.

When all slot occupied, the worker stops consuming messages until a slot becomes available.

#### Operational signals

The system monitors:

- oldest message age by channel;
- queue depth and queue growth rate;
- estimated queue-drain time;
- token-bucket utilization;
- active provider requests;
- provider `429`, timeout, and `5xx` rates;
- retry traffic as a percentage of total traffic;
- rejected and expired notifications.
